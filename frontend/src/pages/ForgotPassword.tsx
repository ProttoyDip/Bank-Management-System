      ...
      <Box sx={{ position: "relative", zIndex: 10, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", px: 2, py: { xs: 12, md: 0 } }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ width: "100%", maxWidth: 450 }}>
          <Box sx={{ bgcolor: "rgba(255,255,255,0.08)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: "24px", p: { xs: 3, md: 5 }, boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.4)" }}>
            {/* Back Button */}
            <Button startIcon={<ArrowBackIcon />} onClick={() => navigate("/login")} sx={{
            ...
            <Box sx={{ display: "flex", gap: 1.5, mb: 4 }}>
              {roles.map((role) => {
                ...
                return (
                  <motion.button key={role.id} type="button" onClick={() => setSelectedRole(role.id)} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} style={{
                    flex: 1,
                    padding: "12px 8px",
                    borderRadius: "12px",
                    border: `2px solid ${isSelected ? "#06b6d4" : "rgba(255,255,255,0.2)"}`,
                    ...
                    <Icon sx={{ fontSize: 20, color: isSelected ? "#06b6d4" : "rgba(255,255,255,0.6)" }} />
                    <span style={{ fontSize: "0.75rem", fontWeight: 500, color: isSelected ? "#06b6d4" : "rgba(255,255,255,0.6)" }}>
                      {role.label}
                    </span>
                  </motion.button>
                );
            ...
            <AnimatePresence>
              ...
              {success && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} style={{ marginBottom: 16, padding: "12px", background: "rgba(34, 197, 94, 0.2)", border: "1px solid rgba(34, 197, 94, 0.3)", borderRadius: "12px", color: "#86efac", fontSize: "0.875rem" }}>
                  {success}
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <Box>
                ...
                <Box sx={{ position: "relative" }}>
                  <MailIcon sx={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "rgba(255,255,255,0.4)", fontSize: 20 }} />
              ...
              <motion.button type="submit" disabled={loading} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} style={{
                width: "100%",
                padding: "14px",
